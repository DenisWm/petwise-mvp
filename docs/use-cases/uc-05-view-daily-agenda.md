...
4. The user optionally chooses filters (e.g., only ACTIVE and CRECHE)
...
7. The backend returns a paginated list of appointments with pet and tutor information
...
- **API Endpoint:** `GET /api/v1/appointments?date=YYYY-MM-DD&status=&type=`
...
|:----------|:-----|:---------|:------------|
| `date` | String (YYYY-MM-DD) | Yes | The date to view |
| `status` | Enum | No | Filter by status (PENDING, ACTIVE, COMPLETED, CANCELED) |
| `serviceType` | Enum | No | Filter by service type (CRECHE, HOTEL) |
